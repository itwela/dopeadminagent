...
import { attomDbTools, connectToAttom } from '@/lib/attom-db';
...
export async function GET() {
  try {
    ...
    // Get all collections
    const collections = await attomDbTools.listCollections();

    // Get basic stats for each collection (limited to first 10)
    const collectionStats = await Promise.all(
      collections.slice(0, 10).map(async (collectionName) => {
        try {
          const info = await attomDbTools.getCollectionInfo(collectionName);
          return {
            name: collectionName,
